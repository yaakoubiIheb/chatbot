...
namespace Chatbot.Dao
{
    public class RuleDao
    {
        ...
        public static List<Rule> autocomplete(string message, int functionId)
        {
            using (SqlConnection conn = new SqlConnection(connection.connectionString))
            {
                List<Rule> rules = new List<Rule>();
                ...
                if (functionId != -1)
                {
                    select = new SqlCommand("SELECT DISTINCT r.* " +
    "FROM [Chatbot].[dbo].[Rule] r, Function_Rule fr,[Chatbot].[dbo].[Trigger] t " +
    "WHERE r.Id = t.RuleID AND r.Id = fr.RuleID AND fr.FunctionID = @functionId AND t.Message like @message", conn);
                ...
                {
                    select = new SqlCommand("SELECT DISTINCT r.* " +
                        "FROM [Chatbot].[dbo].[Rule] r,[Chatbot].[dbo].[Trigger] t " +
                        "WHERE r.Id = t.RuleID AND t.Message like @message", conn);
                    select.Parameters.AddWithValue("@message", '%' + message + '%');
                }
                SqlDataReader rdr = select.ExecuteReader();
                while (rdr.Read())
